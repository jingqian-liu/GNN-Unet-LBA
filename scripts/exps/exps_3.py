...
def main(args):
    ...
    for i, seed in enumerate(seeds):
        ...
        proc = subprocess.Popen(
            ...
            shell=True, stdout=subprocess.PIPE, stderr=None, text=True,
        )
        lines = []
        ...
        if best_ckpt is None:
            print_log(f'WARNING: no checkpoint found for seed {seed} (training may have crashed). Skipping inference and evaluation.')
            print_log(f'The {i}-th experiment skipped!')
            print()
            continue
        ...
        # Mirror what train.sh does: set CUDA_VISIBLE_DEVICES to the physical GPU numbers
        # and use logical index 0, so inference sees the right device regardless of how
        # the parent process was launched (e.g. CUDA_VISIBLE_DEVICES=1 ... --gpus 1).
        ...
        namespace = Namespace(
            ...
            batch_size=32,
            num_workers=4,
            gpu=0  # logical index 0 after CUDA_VISIBLE_DEVICES remapping
        )
