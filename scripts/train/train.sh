...
########## start training ##########
# Build 0-indexed GPU array for train.py; CUDA_VISIBLE_DEVICES already maps
# physical GPU ids to logical indices starting at 0.
GPU_IDX_ARR=()
for i in "${!GPU_ARR[@]}"; do
    GPU_IDX_ARR+=($i)
done
...
cd $CODE_DIR
${PREFIX} train.py --gpus ${GPU_IDX_ARR[@]} ${CONFIG}