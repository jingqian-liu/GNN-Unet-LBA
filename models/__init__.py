    ...
    if 'pretrain' in args.task.lower():
        ...
        if args.pretrain_ckpt:
            ...
            return Model(
                ...
                k_neighbors=args.k_neighbors,
                n_layers=args.n_layers,
                n_residue_layers=getattr(args, 'n_residue_layers', 2),
                n_head=args.n_head,
                atom_level=args.atom_level,
                hierarchical=args.hierarchical,
                no_block_embedding=args.no_block_embedding,
                gate_type=getattr(args, 'gate_type', 'scalar'),
                fps_ratio=getattr(args, 'fps_ratio', 0.5),
                pool_k=getattr(args, 'pool_k', 9),
                **add_params
            )
