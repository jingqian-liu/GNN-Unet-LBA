...
class SchNetUNetEncoderV20(nn.Module):
    """
    V19 + V16: interface-biased top-k selection followed by geometry-aware
    attention pooling at each downsampling step.

    Downsampling pipeline per level:
      1. Score each atom by interface proximity (V16):
             d_i     = min distance to atoms in the opposite segment
             score_i = exp(-d_i / tau)
         Keep exactly k = round(N_g * fps_ratio) highest-scoring atoms per graph.

      2. Each selected center aggregates a residual message from its pool_k
         nearest fine-level neighbours via attention pooling (V19):
             alpha_cj = softmax_j( pool_attn_mlp([h_c, h_j, RBF(d_cj)]) )
             msg_c    = Σ_j alpha_cj * value_lin(h_j)
             h_coarse = h_center + sigmoid(pool_gate) * msg_c

    pool_gate is a per-level scalar initialised to -2 so that
    sigmoid(-2) ≈ 0.12: the model starts near a plain copy of the selected
    centre feature and gradually opens the pooling if beneficial.

    segment_ids (0 = protein, 1 = ligand) must be forwarded so that the
    interface score can be computed at every coarser level.

    All other design choices (GeoAttn backbone, conservative skip-gate init)
    are identical to V14/V19.
    """

    def __init__(self, hidden_size, edge_size, n_layers=6,
                 fps_ratio=0.5, k_neighbors=9, interp_k=3, pool_k=9,
                 gate_type='scalar', tau=3.0):
        super().__init__()
        ...
        # ── Skip-connection gate (identical to V14/V19) ───────────────────
    ...
    @torch.no_grad()
    def _biased_sample(self, pos, batch, segment_ids):
        """
        Interface-biased deterministic top-k sampling (from V16).

        score_i = exp(-d_interface_i / tau), keep top-k per graph
        where k = round(N_g * fps_ratio).
        """
        N        = pos.shape[0]
        device   = pos.device
        n_graphs = batch.max().item() + 1

        d_interface = torch.full((N,), fill_value=self.tau * 20, device=device)

        for g in range(n_graphs):
            g_mask = batch == g
            g_idx  = g_mask.nonzero(as_tuple=True)[0]
            g_pos  = pos[g_idx]
            g_seg  = segment_ids[g_idx]

            is_lig  = g_seg == 1
            l_local = is_lig.nonzero(as_tuple=True)[0]
            p_local = (~is_lig).nonzero(as_tuple=True)[0]

            if l_local.numel() == 0 or p_local.numel() == 0:
                continue  # no interface → uniform score, top-k = first k

            dist_mat = torch.cdist(g_pos[p_local], g_pos[l_local])  # [Np, Nl]
            d_interface[g_idx[p_local]] = dist_mat.min(dim=1).values
            d_interface[g_idx[l_local]] = dist_mat.min(dim=0).values

        score    = torch.exp(-d_interface / self.tau)
        selected = []
        for g in range(n_graphs):
            g_mask = batch == g
            g_idx  = g_mask.nonzero(as_tuple=True)[0]
            k      = min(max(1, round(g_idx.numel() * self.fps_ratio)), g_idx.numel())
            topk_local = torch.topk(score[g_idx], k, largest=True, sorted=False).indices
            selected.append(g_idx[topk_local])

        return torch.cat(selected)

    def _biased_attn_pool(self, h, pos, batch, segment_ids, level_idx):
        """
        Interface-biased selection (V16) followed by attention pooling (V19).

        Returns (idx, h_coarse):
          idx      — selected global node indices
          h_coarse — h_center + sigmoid(pool_gate) * pooled_msg
        """
        # ── 1. Interface-biased top-k selection ───────────────────────────
        if segment_ids is not None:
            idx = self._biased_sample(pos, batch, segment_ids)
        else:
            # fallback: per-graph uniform random sampling
            n_graphs = batch.max().item() + 1
            selected = []
            for g in range(n_graphs):
                g_mask = batch == g
                g_idx  = g_mask.nonzero(as_tuple=True)[0]
                k      = min(max(1, round(g_idx.numel() * self.fps_ratio)), g_idx.numel())
                perm   = torch.randperm(g_idx.numel(), device=pos.device)[:k]
                selected.append(g_idx[perm.sort().values])
        ...
        # ── 2. kNN: for each center find pool_k nearest fine nodes ────────
        ...
        dist = (pos_center[center_idx] - pos[fine_idx]).norm(dim=-1)
        rbf  = self.schnet.distance_expansion(dist)
        ...
        score = self.pool_attn_mlps[level_idx](attn_input)
        alpha = scatter_softmax(score, center_idx, dim=0)
        ...
        gate     = torch.sigmoid(self.pool_gates[level_idx])
    ...
    def forward(self, H, Z, block_id, batch_id, edges, edge_attr=None,
        ...
        assert segment_ids is None or segment_ids.shape[0] == H.shape[0], (
            f'segment_ids length ({segment_ids.shape[0]}) != number of nodes '
            f'after scatter ({H.shape[0]}). Make sure segment_ids is at the '
            f'same granularity as H.'
        )
        ...
        for level in range(self.n_levels):
            ...
            if level < self.n_levels - 1:
                ...
                enc_seg.append(cur_seg)

                # Interface-biased selection + attention pooling
                idx, cur_h = self._biased_attn_pool(
                    cur_h, cur_pos, cur_batch, cur_seg, level
                )
                cur_pos   = cur_pos[idx]
                cur_batch = cur_batch[idx]
                cur_seg   = cur_seg[idx] if cur_seg is not None else None
