...
class SchNetUNetEncoderV21(nn.Module):
    """
    V19 + separate FPS for protein and ligand at each downsampling step.

    Identical to V19 in every way except that the FPS inside
    _fps_with_attn_pool runs independently on protein atoms and ligand atoms
    (each at fps_ratio), then merges the two selected sets.  This ensures
    that both groups are sampled at the same ratio regardless of their
    relative sizes, matching the behaviour of V18.

    segment_ids (0 = protein, 1 = ligand) must be passed to forward() and
    is tracked through all coarser levels.
    """
    ...
    def _separate_fps(self, pos, batch, segment_ids):
        """
        FPS applied separately to protein (seg=0) and ligand (seg=1) atoms.
        Falls back to standard fps over all atoms if one segment is absent.
        """
        is_lig   = segment_ids == 1
        prot_idx = (~is_lig).nonzero(as_tuple=True)[0]
        lig_idx  = is_lig.nonzero(as_tuple=True)[0]

        if prot_idx.numel() == 0 or lig_idx.numel() == 0:
            return fps(pos, batch, ratio=self.fps_ratio, random_start=False)

        prot_sel_local = fps(pos[prot_idx], batch[prot_idx], ratio=self.fps_ratio, random_start=False)
        prot_sel       = prot_idx[prot_sel_local]

        lig_sel_local  = fps(pos[lig_idx], batch[lig_idx], ratio=self.fps_ratio, random_start=False)
        lig_sel        = lig_idx[lig_sel_local]

        return torch.cat([prot_sel, lig_sel]).sort().values

    def _fps_with_attn_pool(self, h, pos, batch, segment_ids, level_idx):
        """
        Separate-FPS downsampling followed by geometry-aware attention pooling.

        Returns (idx, h_coarse):
          idx      — selected global node indices (used to update pos/batch)
          h_coarse — coarse features: h_center + sigmoid(pool_gate) * pooled_msg
        """
        # ── 1. Separate FPS selection ─────────────────────────────────────
        idx          = self._separate_fps(pos, batch, segment_ids)
        h_center     = h[idx]
        pos_center   = pos[idx]
    ...
    def forward(self, H, Z, block_id, batch_id, edges, edge_attr=None,
        ...
        for level in range(self.n_levels):
            ...
            if level < self.n_levels - 1:
                ...
                idx, cur_h = self._fps_with_attn_pool(
