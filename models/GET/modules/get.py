...
class GET(nn.Module):
    ...
    def forward(self, H, Z, block_id, batch_id, edges, edge_attr=None, cached_unit_edge_info=None):
        ...
        #     (unit_row, unit_col), _ = cached_unit_edge_info
        #     Eu = unit_row.shape[0]
        #     Eb = edges.shape[1]
        #     print(f'[Unit edges]  E_u={Eu} (from E_b={Eb} block edges, sparse_k={self.sparse_k})', file=sys.stderr)
        #     print(f'              avg unit-pairs per block-edge = {Eu / max(Eb, 1):.1f}', file=sys.stderr)
