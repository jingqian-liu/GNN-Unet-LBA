...
class SchNetUNetEncoderV13(nn.Module):
    """
    UNet-style encoder:
      - First encoder level (level=0, all-atom graph): GeoAttnInteractionBlock x2
      - All other encoder levels (level>0, coarser graphs): InteractionBlock x2
      - All decoder blocks: InteractionBlock (plain SchNet)
      - Gated skip connections (same as V11/V12)

    gate_type: 'scalar'  → gate ∈ [N_fine, 1]
               'channel' → gate ∈ [N_fine, hidden]
    """
    ...
    def __init__(self, hidden_size, edge_size, n_layers=6,
        ...
        # Build backbone via SchNetGeoAttn to get distance_expansion, cutoff,
        # and the first 2 GeoAttn interaction blocks (level=0).
        self.schnet = SchNetGeoAttn(hidden_size, num_interactions=n_layers,
                                    num_gaussians=self.num_gaussians)
        # Replace interaction blocks at level>0 (indices 2+) with plain InteractionBlock.
        for i in range(2, n_layers):
            self.schnet.interactions[i] = InteractionBlock(
        ...
        # Decoder: all plain InteractionBlock
    ...
    def forward(self, H, Z, block_id, batch_id, edges, edge_attr=None):
        ...
        # ── Encoder ──────────────────────────────────────────────────────
        # level=0 → GeoAttnInteractionBlock; level>0 → InteractionBlock
