...
class SchNetUNetEncoderV5(nn.Module):
    """
    SchNet-U-Net with real pooling.

    Changes from V8:
        1. Remove attention readout.
        2. Replace FPS-only subsampling with FPS-center pooling:
            fine nodes -> nearest FPS centers -> weighted aggregation.
    """
    ...
    def _pool(self, h_fine, pos_fine, batch_fine):
        """
        Real pooling:
            1. FPS selects coarse centers.
            2. Each fine node is assigned to nearby coarse centers.
            3. Coarse feature is weighted aggregation of fine features.
        ...
        # assign fine nodes to nearby coarse centers
        assign = knn(
            pos_coarse,
            pos_fine,
            self.pool_k,
        ...
        feat_score = self.pool_score(h_fine[fine_idx]).squeeze(-1)

        # combine geometry + feature score
        raw_weight = dist_weight * torch.sigmoid(feat_score)

        weight = raw_weight / scatter_sum(
            raw_weight,
            coarse_idx,
            dim=0,
            dim_size=pos_coarse.shape[0],
        )[coarse_idx].clamp(min=1e-9)

        h_coarse = scatter_sum(
            weight.unsqueeze(-1) * h_fine[fine_idx],
            coarse_idx,
            dim=0,
            dim_size=pos_coarse.shape[0],
        )

        # optional residual center feature injection
        h_coarse = h_coarse + h_fine[center_idx]
    ...
    def forward(self, H, Z, block_id, batch_id, edges, edge_attr=None):
        ...
        for level in range(self.n_levels):
            ...
            if level < self.n_levels - 1:
                ...
                cur_h, cur_pos, cur_batch = self._pool(
                    cur_h,
                    cur_pos,
                    cur_batch,
                )
        ...
        # Original normalized-sum readout
