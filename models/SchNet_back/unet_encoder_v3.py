...
class SchNetUNetEncoderV3(nn.Module):
    """
    UNet-style SchNet encoder with separate encoder/decoder weights and attention pooling.

    Identical to V2 except the final graph-level pooling uses learned per-atom attention
    weights (softmax-normalized) instead of sum pooling.

    Attention: score_i = Linear(h_i → 1), weight_i = softmax over atoms in same graph,
               graph_repr = sum(weight_i * h_i)
    """
    ...
    def __init__(self, hidden_size, edge_size, n_layers=6,
        ...
        # Attention pooling: scalar score per atom
        self.attn_lin = nn.Linear(hidden_size, 1)
    ...
    def forward(self, H, Z, block_id, batch_id, edges, edge_attr=None):
        ...
        # Softmax-normalized attention weights per atom within each graph
        scores     = self.attn_lin(block_repr).squeeze(-1)          # [N]
        attn       = scatter_softmax(scores, batch_id, dim=0)        # [N]
        graph_repr = scatter_sum(attn.unsqueeze(-1) * block_repr,
                                 batch_id, dim=0)                    # [B, h]
