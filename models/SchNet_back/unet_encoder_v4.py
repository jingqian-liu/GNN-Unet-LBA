...
class SchNetUNetEncoderV4(nn.Module):
    """
    UNet-style SchNet encoder with attention-based downsampling.

    Identical to V2 except downsampling replaces simple FPS feature-copy with
    attention aggregation: each coarse node (FPS position) gathers features from
    its k nearest fine atoms weighted by learned attention scores.

        h_coarse[i] = sum_j( softmax(attn_lin(h_fine[j])) * h_fine[j] )
                      where j runs over the k nearest fine atoms to coarse node i

    This lets the model learn which fine atoms are most informative to retain
    at each coarser resolution level.
    """

    def __init__(self, hidden_size, edge_size, n_layers=6,
                 fps_ratio=0.5, k_neighbors=9, interp_k=3, down_k=9):
        super().__init__()
        ...
        # Attention scorer for downsampling aggregation
        self.down_attn_lin = nn.Linear(hidden_size, 1)
    ...
    def _downsample(self, h, pos, batch):
        """
        FPS selects K coarse positions. Each coarse node aggregates features
        from its down_k nearest fine atoms via learned attention weights.
        Returns coarse (h, pos, batch).
        """
        idx         = fps(pos, batch, ratio=self.fps_ratio)  # [K]
        pos_coarse  = pos[idx]
        batch_coarse = batch[idx]

        # For each coarse node find its down_k nearest fine atoms
        assign      = knn(pos, pos_coarse, self.down_k,
                          batch_x=batch, batch_y=batch_coarse)
        coarse_idx  = assign[0]   # [K * down_k], indices into coarse set
        fine_idx    = assign[1]   # [K * down_k], indices into fine set

        # Attention weights over fine atoms per coarse node
        scores      = self.down_attn_lin(h[fine_idx]).squeeze(-1)      # [K * down_k]
        attn        = scatter_softmax(scores, coarse_idx, dim=0)        # [K * down_k]
        h_coarse    = scatter_sum(attn.unsqueeze(-1) * h[fine_idx],
                                  coarse_idx, dim=0,
                                  dim_size=pos_coarse.shape[0])        # [K, hidden]
    ...
    def forward(self, H, Z, block_id, batch_id, edges, edge_attr=None):
        ...
        for level in range(self.n_levels):
            ...
            if level < self.n_levels - 1:
                ...
                # Attention aggregation downsampling
                cur_h, cur_pos, cur_batch = self._downsample(cur_h, cur_pos, cur_batch)
