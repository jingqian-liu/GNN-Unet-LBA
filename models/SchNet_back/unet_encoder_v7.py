...
class SchNetUNetEncoderV7(nn.Module):
    """
    ...
    def __init__(self, hidden_size, edge_size, n_layers=6,
        ...
        self.edge_gate = (
           nn.Sequential(
            nn.Linear(self.num_gaussians + edge_size, self.num_gaussians),
            nn.SiLU(),
            nn.Linear(self.num_gaussians, self.num_gaussians),
            nn.Sigmoid(),
    ...
    def forward(self, H, Z, block_id, batch_id, edges, edge_attr=None):
        ...
        if edge_attr is not None and self.edge_gate is not None:
            edge_input = torch.cat([rbf0, edge_attr], dim=-1)
            gate = self.edge_gate(edge_input)
            delta = self.edge_delta(edge_input)
            rbf0 = rbf0 + gate * delta
